...
void ImageLabelmapDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK((height == gt_height) && (width == gt_width)) << "groundtruth("<<lines_[lines_id_].second<<") size != image"<<lines_[lines_id_].first<<" size"; 
...
void ImageLabelmapDataLayer<Dtype>::load_batch(LabelmapBatch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    CHECK((height == gt_height) && (width == gt_width)) << "GT image("<<lines_[lines_id_].second<<") size should be equal to true image("<<lines_[lines_id_].first<<") size";
    CHECK(gt_channels == 1) << "GT image channel number should be 1";
    ...
    this->data_transformer_->LabelmapTransform(cv_gt, &(this->transformed_labelmap_), h_off, w_off, do_mirror);
    ...
    for(int zk_i=0;zk_i<zk_count;zk_i++){
      ...
      else if(transformat_data[zk_i] >= 10 && transformat_data[zk_i] <26)
        transformat_data[zk_i] = Dtype(2);
      
      else if(transformat_data[zk_i] >= 26 && transformat_data[zk_i] < 60)
        transformat_data[zk_i] = Dtype(3);
      
      else if(transformat_data[zk_i] >= 60 && transformat_data[zk_i] < 150)
        transformat_data[zk_i] = Dtype(4);
      else
        LOG(FATAL)<<"UNKNOWN SCALE";  
    }
